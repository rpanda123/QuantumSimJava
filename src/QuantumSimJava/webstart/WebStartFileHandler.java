...
/**
 * Handles loading files from the local filesystem using the Java WebStart
 * API, which asks the user for permissions for individual file reads and
 * writes if the application is unsigned.
 * 
 * Insulates the rest of the code from the Java WebStart API so that if
 * we're not running in a WebStart environment and the javax.jnlp API
 * is not available, the system will still run by not making a reference
 * to this class and thereby not trying to import missing classes.
 * @author Dave Boden
 * @version 1.0
 */
public class WebStartFileHandler {
    ...
    * @param fileDirectoryHint the most recently used file directory
   ...
   public WebStartFileContents getFileContents(String fileDirectoryHint)
      ...
      try {
         ...
         FileContents fileContents = fos.openFileDialog(fileDirectoryHint, null);
    ...
    * @param fileDirectoryHint the directory into which the file is to be saved
    * @param webStartFileContents the file contents to be saved
    * @return true if and only if the file contents could be saved successfully
    * @throws IOException
    * @throws WebStartFileHandlerNotAvailableException
    */
   public boolean saveFileContents(String fileDirectoryHint, WebStartFileContents webStartFileContents)
           throws IOException, WebStartFileHandlerNotAvailableException {
      try {
         ...
         FileContents fileContents = fss.saveFileDialog(
                 fileDirectoryHint, null, webStartFileContents.getInputStream(),
    ...
    * @param file the file the webs start content is to be read of
    * @return the contents of web start file
    * @throws IOException
    * @throws WebStartFileHandlerNotAvailableException
    */
   public WebStartFileContents openFile(File file)
             throws IOException, WebStartFileHandlerNotAvailableException {
       try {
           ExtendedService exs = (ExtendedService)ServiceManager.lookup("javax.jnlp.ExtendedService");
